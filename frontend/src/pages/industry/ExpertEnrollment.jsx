...
const ExpertEnrollment = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setSubmitting(true);
      await apiClient.post(`/expert-sessions/industry/${expert.id}/enroll`, payload);
      setSuccess(true);
      // Navigate back to expert sessions page after 2 seconds with refresh parameter
      setTimeout(() => {
        navigate("/industry/expert-sessions?refresh=true");
      }, 2000);
    } catch (err) {
      console.error(err);
      const message =
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-4">
          <Button variant="outline" onClick={() => navigate(`/industry/expert-sessions/${expertId}`)}>
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Details
          </Button>
          <div>
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <FormField
                    label="Industry type"
                    required
                    value={enrollmentForm.industryType}
                    onChange={(event) => onFormChange("industryType", event.target.value)}
                ...
                <div className="flex gap-4 pt-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => navigate(`/industry/expert-sessions/${expertId}`)}
                    className="flex-1"
                  >
                    Cancel
                  </Button>
...
const FormField = ({ label, value, onChange, type = "text", required, icon: Icon, placeholder, ...rest }) => (
  <div className="space-y-1.5">
    <label className="text-sm font-medium text-slate-700">
      {label}
